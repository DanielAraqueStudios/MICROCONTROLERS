...
int main() {
    ...
    // 2. Configurar los pines para el sensor ultrasonido: Trigger en PA3 y Echo en PA2 (antes PA0)
    GPIOA->MODER |= (1<<6);    // PA3 como salida (Trigger)
    GPIOA->MODER &= ~(3<<4);   // PA2 como entrada (Echo) - Cambiado de PA0
    ...
    while (1) {
        ...
        // Cálculo de la distancia ajustado
        Distancia = (tiempo * 0.0343) / 2.0;
        
        // Ajustar el filtro de rango considerando las dimensiones del recipiente
        if (Distancia < 1.0 || Distancia > 500.0) {  // Limitamos a 500cm de altura máxima
            Distancia = DistanciaFiltrada;
        }

        // Filtro más suave para estabilizar la lectura
        DistanciaFiltrada = (DistanciaFiltrada * 0.85) + (Distancia * 0.15);
        DistanciaFiltrada = round(DistanciaFiltrada * 10) / 10.0;

        // Cálculo del volumen ajustado:
        // 7 unidades por cm * distancia
        Volumen = 7.0 * DistanciaFiltrada;
        
        if (Volumen < 0) Volumen = 0;

        UpdateVolumen(Volumen);
        SysTick_Wait1ms(100);  // Aumentamos el tiempo entre lecturas para mayor estabilidad
    }
}
...
uint32_t MeasurePulseWidth(void) {
    ...
    while (!(GPIOA->IDR & (1<<2))) {  // Cambiado de (1<<0) a (1<<2)
        if (timeout-- == 0) {
